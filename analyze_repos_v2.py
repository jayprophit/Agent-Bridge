...
# Each line has tabs separating fields
repos = []
for line in content.strip().split('\n'):
    ...
    if len(parts) >= 5:
        ...
        visibility_type = parts[2] if len(parts) > 2 else ""
        fork_info = parts[3] if len(parts) > 3 else ""
        date = parts[4] if len(parts) > 4 else ""
        
        # Parse visibility - could be "public" or have other info
        visibility = visibility_type.split()[0] if visibility_type else "unknown"
        
        # Parse fork status - "fork" in the string or not
        fork_status = "fork" in fork_info.lower() or " fork" in fork_info.lower()
...
# Known personal project names and keywords
known_personal_names = [
    ...
    "THE-ARCHITECTURE-OF-SHADOWS", "profile", "learning", "learning projects"
]
...
for p in personal_projects:
    ...
    if p['name'] in ["Agent-Bridge", "IDE-WORKSPACE"]:
        print(f"    *** THIS IS THE LOCAL REPO BEING WORKED ON ***")
...
print("=" * 70)
# Show non-obvious ones - skip obvious external projects
shown = 0
for s in standalone_non_fork:
    # Only show repos that aren't obviously external
    obvious_external = ["flatpak", "bubblewrap", "ostree", "tuf", "smithay", "wgpu",
                        ...
                        "signalsmith-stretch", "Ardour", "flat", "redox", "webots",
                        "depthai", "containerd", "energy", "helm", "playwright",
                        "ansible", "vcpkg", "opentelemetry", "supercollider", "edk2",
                        "rust-libp2p", "gatk", "nats", "bazel", "tfhe", "PDAL",
                        "crewAI", "openmm", "nrn", "wasmtime", "code-server",
                        "dagster", "petsc", "openbao", "theia", "llama_index",
                        "compose", "renode", "conan", "firecracker", "warpx",
                        "riscv", "chisel", "automerge", "A2A", "mem0", "ditto",
                        "sofa", "pcl", "pymatgen", "langgraph", "Orekit", "scipy",
                        "tracktion", "tauri", "samtools", "scikit-bio", "peft",
                        "pyOCD", "python-sdk", "duckdb", "gdal", "xterm", "buildkit",
                        "sleap", "qdrant", "gz-sim", "open-webui", "lammps", "ros2",
                        "pyscf", "CRISPResso2", "graphiti", "influxdb", "mediapipe",
                        "openvsx", "agent-framework", "phone-harness", "mujoco", "mne",
                        "neo4j", "sofa", "mattrix", "scikit", "pyOCD", "python-sdk"]
    is_obvious = s["name"].lower() in [x.lower() for x in obvious_external]
    if not is_obvious and shown < 40:
        print(f"  {s['name']} - {s['visibility']} - {s['date']}")
        shown += 1
if shown >= len(standalone_non_fork):
    print(f"  (all {shown} repos shown)")
else:
    print(f"  ... and {len(standalone_non_fork)-shown} more")
