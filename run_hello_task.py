...
bridge_cmd = [sys.executable, r'C:\Users\jpowe\Desktop\Agent-Bridge\cli.py', 
              ...
              'serve', '--host', '127.0.0.1', '--port', '9023']
...
for i in range(10):
    time.sleep(2)
    try:
        with urllib.request.urlopen('http://127.0.0.1:9023/health', timeout=5) as resp:
            health = resp.read().decode()
            print('Bridge healthy on 9023:', health[:100])
            break
...
# Create session and submit task
...
cli = AgentRuntimeClient(base='http://127.0.0.1:9023', token='')
...
# Submit the EXACT task that worked before: "Create a file called test.txt containing exactly HELLO."
task_text = 'Create a file called test.txt containing exactly HELLO.'
tdata = json.dumps({'text': task_text}).encode()
req = urllib.request.Request(
    'http://127.0.0.1:9023/v1/sessions/' + sid + '/tasks', 
    data=tdata, 
    method='POST', 
...
with urllib.request.urlopen(req, timeout=30) as resp2:
    ...
    for i in range(30):
        time.sleep(3)
        try:
            status_req = urllib.request.Request('http://127.0.0.1:9023/v1/sessions/' + sid + '/status')
    ...
    # Get export
    try:
        export_req = urllib.request.Request('http://127.0.0.1:9023/v1/sessions/' + sid + '/export')
