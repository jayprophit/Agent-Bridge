...
cmd = [sys.executable, r'C:\Users\jpowe\Desktop\Agent-Bridge\cli.py', 
       ...
       'serve', '--host', '127.0.0.1', '--port', '9015']
...
# Test health
try:
    with urllib.request.urlopen('http://127.0.0.1:9015/health', timeout=5) as resp:
        print('Bridge healthy on 9015')
...
cli = AgentRuntimeClient(base='http://127.0.0.1:9015', token='')
...
req = urllib.request.Request(
    'http://127.0.0.1:9015/v1/sessions/' + sid + '/tasks', 
    data=tdata, 
    method='POST', 
...
with urllib.request.urlopen(req, timeout=30) as resp2:
    ...
    for i in range(12):
        time.sleep(3)
        status_req = urllib.request.Request('http://127.0.0.1:9015/v1/sessions/' + sid + '/status')
        with urllib.request.urlopen(status_req, timeout=15) as resp3:
            ...
            if task_status in ('COMPLETED', 'FAILED', 'CANCELLED', 'ROLLED_BACK', 'INTERRUPTED'):
                ...
                try:
                    export_req = urllib.request.Request('http://127.0.0.1:9015/v1/sessions/' + sid + '/export?task=' + tid + '&format=markdown')
